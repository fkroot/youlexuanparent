...
public class SeckillGoodsServiceImpl implements SeckillGoodsService {
	...
	public List<TbSeckillGoods> findList() {
		...
		if(null == tbSeckillGoods || tbSeckillGoods.size() == 0){
			...
			List<TbSeckillGoods> tbSeckillGoods1 = seckillGoodsMapper.selectByExample(tbSeckillGoodsExample);
			System.out.println("将秒杀商品放入缓存中");
			for (TbSeckillGoods tbSeckillGood : tbSeckillGoods1) {
				redisTemplate.boundHashOps("seckillGoods").put(tbSeckillGood.getId(),tbSeckillGood);
			}

			return tbSeckillGoods1;
