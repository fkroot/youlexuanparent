...
import com.offcn.mapper.TbAreasMapper;
import com.offcn.mapper.TbCitiesMapper;
import com.offcn.mapper.TbProvincesMapper;
...
public class AddressServiceImpl implements AddressService {
    ...
    public List<TbCities> findCityList(String provinceId) {
        TbProvinces tbProvinces = tbProvincesMapper.selectByPrimaryKey(Integer.valueOf(provinceId));

        TbCitiesExample example = new TbCitiesExample();
        TbCitiesExample.Criteria criteria = example.createCriteria();
        criteria.andProvinceidEqualTo(tbProvinces.getProvinceid());
        return tbCitiesMapper.selectByExample(example);
    }

    @Override
    public List<TbAreas> findAreasList(String cityId) {
        TbCities tbCities = tbCitiesMapper.selectByPrimaryKey(Integer.valueOf(cityId));

        TbAreasExample example = new TbAreasExample();
        TbAreasExample.Criteria criteria = example.createCriteria();
 criteria.andCityidEqualTo(tbCities.getCityid());
